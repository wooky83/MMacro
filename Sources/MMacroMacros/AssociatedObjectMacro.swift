...
extension AssociatedObjectMacro: PeerMacro {
    public static func expansion(of node: AttributeSyntax,
        ...
        guard let varDecl = declaration.as(VariableDeclSyntax.self),
            ...
            throw MacroError("`@AssociatedObject` must be appended to the property declaration.")
        }
...
extension AssociatedObjectMacro: AccessorMacro {
    public static func expansion(of node: AttributeSyntax,
        ...
        guard let type = binding.typeAnnotation?.type else {
            throw MacroError("Specify a type explicitly")
        }

        if binding.accessorBlock != nil{
            throw MacroError("`accessor should not be specified.")
        }
        
        guard case let .argumentList(arguments) = node.arguments,
              let firstElement = arguments.first?.expression,
              let policy = firstElement.as(MemberAccessExprSyntax.self) else {
            throw MacroError("`must be objc_AssociationPolicy specified.")
        }
